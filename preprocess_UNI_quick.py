# Adapted from https://github.com/AIRMEC/im4MEC/blob/main/preprocess.py
...
class BagOfTiles(Dataset):
    def __init__(self, wsi, tiles, resize_to=224):
        ...
        # self.roi_transforms = image_processor
...
def extract_features(model, device, wsi, filtered_tiles, workers, out_size, batch_size):
    ...
    with torch.no_grad():
        for batch, coords in loader:
            ...
            # features = outputs.last_hidden_state[:, 0, :].detach().cpu().numpy()  # (1, 1024) shape
            yield outputs, coords
            

def get_wsi_features(device, wsi, out_size, batch_size):
    kwargs = (
        {"num_workers": workers, "pin_memory": True} if device.type == "cuda" else {}
    )
    local_dir = "./UNI/assets/ckpts/vit_large_patch16_224.dinov2.uni_mass100k/"
...
if __name__ == "__main__":
    ...
    slide_ids = [os.path.splitext(f)[0] for f in os.listdir(args.input_dir) if f.lower().endswith('.tif')]
    # slide_ids = [os.path.splitext(f)[0] for f in os.listdir(args.input_dir) if f.lower().endswith('.svs')]
    print(len(slide_ids))
    ...
    for slide_id in slide_ids:
        ...
        if os.path.exists(output_file_path):
            ...
            print(f"Skipping {output_file_path}, it already exists.")
            continue  
        # Open the slide for reading
        slide_path = os.path.join(args.input_dir, slide_id + '.tif')
        # slide_path = os.path.join(args.input_dir, slide_id + '.svs')
        ...
        try:
            ...
            if str(e) == 'microns per pixel along X-dimension not available':
                print(f"Skipping slide {slide_id} due to missing MPP_X property")
                continue
            else:
                raise e

...
def _get_split_loader(args, split_dataset, training = False, testing = False, weighted = False, batch_size=1):
    ...
    """
    if device.type == "cuda":
        if args.modality in ["pre_adapter", "pre", "dlr_adapter", "dlr", "dlp_adapter", "dlp", "abmil_wsi", "pre_cli", "post_cli", "cli"]:
            adapter_workers = max(0, int(getattr(args, "adapter_num_workers", 2)))
            kwargs = {'num_workers': adapter_workers, 'pin_memory': True}
        else:
    ...
    elif args.modality in ["survpath", "dlrp", "pre_adapter", "pre", "dlr_adapter", "dlr", "dlp_adapter", "dlp", "abmil_wsi", "pre_cli", "post_cli", "cli"]:

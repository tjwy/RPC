#----> pytorch imports
import torch
import warnings
warnings.filterwarnings('ignore', category=RuntimeWarning, message='.*invalid value encountered in scalar divide.*')
warnings.filterwarnings(
    'ignore',
    category=FutureWarning,
    message=r'.*torch\.cuda\.amp\..*',
)
#----> general imports
import pandas as pd
import numpy as np
import numpy
import pdb
import os
from timeit import default_timer as timer
# from datasets.dataset_survival_pre import SurvivalDatasetFactory
from datasets.dataset_survival_latest import SurvivalDatasetFactory

from utils.core_utils_latest1 import _train_val
from utils.file_utils import _save_pkl
from utils.general_utils_fufa import _get_start_end, _prepare_for_experiment
import random
from utils.process_args import _process_args

numpy.random.seed(42)
random.seed(42)
torch.manual_seed(42)

def main(args):

    #----> prep for 5 fold cv study
    folds = _get_start_end(args)
    
    #----> storing the val and test cindex for 5 fold cv
    all_val_iauc = []
    all_val_loss = []
    all_acc = []
    all_train_metrics = []
    all_val_metrics = []
    # final_metrics_df
    final_metrics_df = pd.DataFrame()

    all_confusion_values = []

    for i in folds:
        datasets = args.dataset_factory.return_splits(
            args,
            csv_path='{}/splits_{}.csv'.format(args.split_dir, i),
            fold=i
        )
        
        print("Created train and val datasets for fold {}".format(i))

        # results, ( train_metrics_dict,val_metrics_dict,val_iauc, acc,total_loss,confusion_values) = _train_val(datasets, i, args)
        results, ( train_metrics_dict,val_metrics_dict,val_iauc, acc,total_loss) ,train_results = _train_val(datasets, i, args)
        all_val_iauc.append(val_iauc)
        all_acc.append(acc)
        all_val_loss.append(total_loss)
        all_train_metrics.append(train_metrics_dict)
        all_val_metrics.append(val_metrics_dict)

        # all_confusion_values.append(confusion_values)  # : 
        # Assuming `results` is a dictionary
        df = pd.DataFrame(results)
        # Save DataFrame to CSV
        filename = os.path.join(args.results_dir, 'split_{}_results.csv'.format(i))
        df.to_csv(filename, index=False)
        
        #write results to pkl
        filename = os.path.join(args.results_dir, 'split_{}_results.pkl'.format(i))
        print("Saving results...")
        _save_pkl(filename, results)
        # DataFrame
                # CSV
        train_df = pd.DataFrame({
            'case_id': train_results['case_id'],
            'prob': train_results['probs'],
            'label': train_results['labels'],
            'pred': train_results['predictions']
        })
        train_filename = os.path.join(args.results_dir, 'split_{}_train_results.csv'.format(i))
        train_df.to_csv(train_filename, index=False)
        train_metrics_series = pd.Series({f'train_{key}': f"{train_metrics_dict[key][0]:.3f} (95% CI: {train_metrics_dict[key][1]:.3f}, {train_metrics_dict[key][2]:.3f})" for key in train_metrics_dict})
        val_metrics_series = pd.Series({f'val_{key}': f"{val_metrics_dict[key][0]:.3f} (95% CI: {val_metrics_dict[key][1]:.3f}, {val_metrics_dict[key][2]:.3f})" for key in val_metrics_dict})
        
        # train_cm, val_cm = confusion_values
        # confusion_series = pd.Series({
        #     'train_TP': train_cm[1, 1],
        #     'train_FP': train_cm[0, 1],
        #     'train_TN': train_cm[0, 0],
        #     'train_FN': train_cm[1, 0],
        #     'val_TP': val_cm[1, 1],
        #     'val_FP': val_cm[0, 1],
        #     'val_TN': val_cm[0, 0],
        #     'val_FN': val_cm[1, 0]
        # })
        # # #  final_metrics_df
        # # metrics_df = pd.DataFrame({
        # #     'folds': [f'Fold {i+1}'],
        # #     **train_metrics_series.to_dict(),
        # #     **val_metrics_series.to_dict(),
        # # })
        # # final_metrics_df = pd.concat([final_metrics_df, metrics_df], ignore_index=True)
        # final_metrics_df
        metrics_df = pd.DataFrame({
            'folds': [f'Fold {i+1}'],
            **train_metrics_series.to_dict(),
            **val_metrics_series.to_dict(),
            # **confusion_series.to_dict()  # : 
        })
        final_metrics_df = pd.concat([final_metrics_df, metrics_df], ignore_index=True)

    final_df = pd.DataFrame({
        'folds': folds,
        'val_iauc': all_val_iauc,
        "val_loss": all_val_loss,
        "val_acc" : all_acc,
    })
            # Calculate the mean of val_iauc
    val_iauc_mean = np.mean(all_val_iauc)
    print("Mean val_iauc:", val_iauc_mean)
            # Add mean val_iauc to the final_df
    final_df.loc[len(final_df)] = ['mean', val_iauc_mean, np.nan, np.nan]
    if len(folds) != args.k:
        partial_start = int(folds[0]) if len(folds) > 0 else args.k_start
        partial_end = int(folds[-1]) + 1 if len(folds) > 0 else args.k_end
        save_name = 'summary_partial_{}_{}.csv'.format(partial_start, partial_end)
    else:
        save_name = 'summary.csv'
        
    final_df.to_csv(os.path.join(args.results_dir, save_name))
    print(type(final_metrics_df))

#     mean_train_metrics = final_metrics_df.filter(like='train_').mean()
#     mean_val_metrics = final_metrics_df.filter(like='val_').mean()
#     mean_metrics = pd.concat([mean_train_metrics, mean_val_metrics])
#     mean_metrics['folds'] = 'Mean'
# #  final_metrics_df
#     final_metrics_df = pd.concat([final_metrics_df, mean_metrics.to_frame().T], ignore_index=True)
    # CSV
    save_name = 'summary_metrics.csv' if len(folds) == args.k else 'summary_metrics_partial.csv'
    final_metrics_df.to_csv(os.path.join(args.results_dir, save_name))

    # # : 
    # confusion_df = final_metrics_df[['folds', 'train_TP', 'train_FP', 'train_TN', 'train_FN', 
    #                                  'val_TP', 'val_FP', 'val_TN', 'val_FN']]
    # confusion_df.to_csv(os.path.join(args.results_dir, 'confusion_matrix_values.csv'))

    # # : 
    # metrics_explanation = pd.DataFrame({
    #     'Metric': ['Sensitivity', 'Specificity', 'PPV', 'NPV'],
    #     'Formula': ['TP/(TP+FN)', 'TN/(TN+FP)', 'TP/(TP+FP)', 'TN/(TN+FN)'],
    #     'Train_Values': [
    #         f"train_TP/(train_TP+train_FN)",
    #         f"train_TN/(train_TN+train_FP)",
    #         f"train_TP/(train_TP+train_FP)",
    #         f"train_TN/(train_TN+train_FN)"
    #     ],
    #     'Val_Values': [
    #         f"val_TP/(val_TP+val_FN)",
    #         f"val_TN/(val_TN+val_FP)",
    #         f"val_TP/(val_TP+val_FP)",
    #         f"val_TN/(val_TN+val_FN)"
    #     ]
    # })
    # metrics_explanation.to_csv(os.path.join(args.results_dir, 'metrics_explanation.csv'))
    print("Metrics summary saved to", os.path.join(args.results_dir, save_name))
 
warnings.filterwarnings("ignore", category=FutureWarning)


if __name__ == "__main__":
    start = timer()
    #----> read the args
    args = _process_args()
    print(f"Use Nystrom approximation: {args.use_nystrom}")
    #----> Prep
    args = _prepare_for_experiment(args)
    args.dataset_factory = SurvivalDatasetFactory(
        study=args.study,
        label_file=args.label_file,
        clinical_file=args.clinical_file,
        rad_data_dir=args.rad_data_dir,
        external_pathology_dir=args.external_pathology_dir,
        tcga_pathology_dir=args.tcga_pathology_dir,
        external_mri_dir=args.external_mri_dir,
        tcga_mri_dir=args.tcga_mri_dir,
        seed=args.seed, 
        print_info=True, 
        # n_bins=args.n_classes, 
        label_col=args.label_col, 
        eps=1e-6,
        num_patches=args.num_patches,
        need_rad_data=(args.modality not in ["dlp_adapter", "dlp", "abmil_wsi", "pre_cli", "post_cli", "cli"]),
        # is_mcat = True if args.modality == "coattn" else False,
        # is_survpath = True if args.modality == "survpath" else False,
        # type_of_pathway=args.type_of_path,
    )

    #---> perform the experiment
    results = main(args)
    # use_nystrom 
    

    #---> stop timer and print
    end = timer()
    print("finished!")
    print("end script")
    print('Script Time: %f seconds' % (end - start))
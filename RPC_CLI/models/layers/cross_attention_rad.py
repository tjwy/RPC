...
class MMAttention(nn.Module):
    ...
    def forward(self, x, mask=None, return_attn=False):
        b, n, _, h, m, eps = *x.shape, self.heads, self.num_slices, self.eps
        ...
        q_pathways = q[:, :, :self.num_slices, :]  # bs x head x num_pathways x dim
        k_pathways = k[:, :, :self.num_slices, :]

        q_histology = q[:, :, self.num_slices:, :]  # bs x head x num_patches x dim
        k_histology = k[:, :, self.num_slices:, :]
        ...
        out_histology = cross_attn_histology @ v[:, :, :self.num_slices]

...
# 'case_id'
case_id_data = raw_data['slide_id']
...
trian_ids = fold_df[fold_df[f'fold-2'] == 'training']['case_id'].tolist()

raw_train_data = raw_data[raw_data['case_id'].astype(str).isin(trian_ids)]
scaler.fit(raw_train_data[numerical_columns])

numerical_data = pd.DataFrame(scaler.transform(raw_data[numerical_columns]),columns=numerical_columns, index=raw_data.index)
...
def gen_dataset_multi_images(data_dir, id_list, fold_df,clinical_df, img_size=224, outline=10):
    ...
    for id in id_list:
        ...
        clinical_list.append(clinical_df[clinical_df['slide_id'].astype(str) == id].values[0][1:])
...
class SeqVIT(nn.Module):
    ...
    def __init__(self, sequence_length=3,clinical_feature_dim=11):
...
test_img_list, test_label_list,test_ids_list ,test_clinical_list= gen_dataset_multi_images(file_dir, X_test_fold, fold_df,clinical_df)

test_y_binary = np.array(test_label_list)
test_img = np.array(test_img_list)
#  DataLoader
test_dataset = CustomDataset(test_img_list, test_label_list, test_ids_list,test_clinical_list,transform=data_transform1)
...
checkpoint_path = os.path.join(checkpoint_dir, f'checkpoint_fold_{fold+1}_0.0001_0.001.pt')
# checkpoint_path = os.path.join(checkpoint_dir, f'checkpoint_fold_{fold+1}_0.0005_0.005.pt')
# checkpoint_path = f'checkpoint_fold_{fold+1}.pt'
...
if fold == 2:
    # formatted_results.append(external_metrics)
    ...
    for metric_name, metric_func in metric_funcs.items():
        ...
        #  CSV 
        with open('final_metrics_formatted_rad_CLI_ex9_2.csv', 'w') as f:
            for result in formatted_results:
                f.write(result + '\n')
        # print(f'Fold {fold + 1} metrics in the specified format saved to {fold_output_file}')
...
# external_metrics_df.to_csv(f'external_metrics_fold_{fold+1}.csv', index=False)
all_external_metrics.append(external_metrics_df)
print(f'External Validation AUC: {external_auc}')
    # scheduler.step()
all_predictions.extend(np.around(external_outputs, 4))
...
# DataFrameCSV
external_metrics_df.to_csv('external_metrics_rad_cli9_2.csv', index=False)

#  fold  CSV 
output_file = 'model_predictions_external_rad_cli9_2.csv'
...
# for i in range(5):
#     print(f"Fold {i + 1} - Train AUC: {all_external_auc[i]:.3f}")

# print("Average results:")

# print(f'Average External AUC: {np.mean(all_external_auc):.3f}')
# print(f'Average External Accuracy: {np.mean(all_external_acc):.3f}')
# print(f'Average External Precision: {np.mean(all_external_precision):.3f}')
# print(f'Average External Recall: {np.mean(all_external_recall):.3f}')
# print(f'Average External Specificity: {np.mean(all_external_specificity):.3f}')
# print(f'Average External PPV: {np.mean(all_external_ppv):.3f}')
# print(f'Average External NPV: {np.mean(all_external_npv):.3f}')
